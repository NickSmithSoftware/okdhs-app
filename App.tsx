...
const uploadFiles = async (images: ImageInfo[], social: string) => {
	...
	try {
		...
		const results = blobs.map((blob, index) => {
			const name = images[index].uri.substring(
				images[index].uri.lastIndexOf("/") + 1
			);
...
const App = () => {
	...
	const [formValidity, setFormValidity] = useState([false, false, false]);
	...
	useEffect(() => {
		setFormValidity(
			Object.entries(form).map((entry: [string, string]) =>
				validate(entry)
			)
		);
	}, [form]);
